...
# -------------------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------------------
def reporte_Tokens_AS(Lexemas_Aceptados):
    nombre = 'Reporte_Tokens_Aceptados_S'
    with open(nombre + '.html', 'w') as html:
        html.write('<html>')
        html.write('    <head>')
        html.write('        <title>Reporte Tokens Aceptados</title>')
        ...
        html.write('    <body >')
        html.write('        <h1><span class="blue"></span>Reporte<span class="blue"></span> <span class="yellow"> Tokens</span></h1>')
        html.write('        <h2> Analisis <a target="_blank">Sintactico</a></h2>')
        html.write('        <table class="container">')
        html.write('            <thead>')
        html.write('                <tr>')
        html.write('                    <th><h1>NO</h1></th>')
        ...
        # -----------------------------------------------------------------------------------------
        i=0
        if (Lexemas_Aceptados != []):
            for al in Lexemas_Aceptados:
                i += 1
                html.write('                <tr>')
                html.write('                    <td>' + str(i) + '</td>')
                html.write('                    <td>' + str(al['valor']) + '</td>')
...
def reporte_Tokens_error(lexema_error):
    ...
    with open(nombre + '.html', 'w') as html:
        ...
        html.write('    </head>')
        html.write('    <body >')
        html.write('        <h1><span class="blue"></span>Reporte<span class="blue"></span> <span class="yellow"> Errores</span></h1>')
